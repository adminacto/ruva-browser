...
pub fn main() {
    ...
    let webview = WebViewBuilder::new()
        ...
        .with_ipc_handler(move |req: Request<String>| { let _ = ipc_tx.send(req.body().to_string()); })
        .build(&window).unwrap();
        ...
        *control_flow = ControlFlow::Wait;
        ...
        while let Ok(json_str) = settings_rx.try_recv() {
            if let Ok(val) = serde_json::from_str::<serde_json::Value>(&json_str) {
                ...
                if let Some(v) = val.get("search_engine").and_then(|x| x.as_str()) { if !v.is_empty() { st.settings.search_engine = v.to_string(); } }
                if let Some(v) = val.get("homepage").and_then(|x| x.as_str()) { st.settings.homepage = v.to_string(); }
                if let Some(v) = val.get("bg_color").and_then(|x| x.as_str()) { st.settings.bg_color = v.to_string(); }
                if let Some(v) = val.get("bg_image").and_then(|x| x.as_str()) { st.settings.bg_image = v.to_string(); }
                if let Some(v) = val.get("bg_video").and_then(|x| x.as_str()) { st.settings.bg_video = v.to_string(); }
                if let Some(v) = val.get("new_tab_show_ntp").and_then(|x| x.as_bool()) { st.settings.new_tab_show_ntp = v; }
                if let Some(v) = val.get("show_tab_bar").and_then(|x| x.as_bool()) { st.settings.show_tab_bar = v; }
                if let Some(v) = val.get("auto_show_tab_bar").and_then(|x| x.as_bool()) { st.settings.auto_show_tab_bar = v; }
                if let Some(v) = val.get("block_fullscreen").and_then(|x| x.as_bool()) { st.settings.block_fullscreen = v; }
                if let Some(v) = val.get("load_images").and_then(|x| x.as_bool()) { st.settings.load_images = v; }
                if let Some(v) = val.get("ai_api_key").and_then(|x| x.as_str()) { if !v.is_empty() { st.settings.ai_api_key = v.to_string(); } }
                if let Some(v) = val.get("ai_model").and_then(|x| x.as_str()) { st.settings.ai_model = v.to_string(); }
                if let Some(ntp) = val.get("ntp") { if let Ok(n) = serde_json::from_value::<NtpSettings>(ntp.clone()) { st.settings.ntp = n; } }
                st.settings.save();
            }
        }
            ...
            if let Ok(msg) = serde_json::from_str::<IpcMsg>(&msg_str) {
                match msg.cmd.as_str() {
                    "navigate" => {
                        ...
                        } else if kb_state.borrow().is_ntp {
                            load_ntp(&kb_state);
                        } else {
                            load_ntp(&kb_state);
                        }
                    ...
                    "ai_chat" => {
                        ...
                        std::thread::spawn(move || {
                            if api_key.is_empty() { let _ = ai_tx_clone.send("\u{26a0}\u{fe0f} API key not set".to_string()); return; }
